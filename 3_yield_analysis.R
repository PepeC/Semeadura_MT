...
m4 <- train(yield ~ geounit + precip_decjan*estimate_corn_xmid + geounit*ndv_mar + tmax_apr,
            data = bra_go_latest_all_mt2,
            method = 'lm',
            #tuneLength = 40,
            na.action = na.omit,
...
pred_tib <- predict(m, newdata = bra_go_latest_all_mt2)

bra_go_latest_all_mt2 %>% bind_cols(pred_tib)
